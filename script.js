...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.requestIdleCallback) {
        window.requestIdleCallback(() => sendCAPIEvent('PageView'));
    } else {
        setTimeout(() => sendCAPIEvent('PageView'), 1000);
    }
});

// ==========================================
// META PIXEL LOGIC (DEFERRED)
// ==========================================
function initMetaPixel() {
    ...
    fbq('init', '1487174426390347');
    fbq('track', 'PageView');
}

// Defer PageView tracking to free up main thread during initial load
if (window.requestIdleCallback) {
    window.requestIdleCallback(initMetaPixel);
} else {
    setTimeout(initMetaPixel, 1000);
}
